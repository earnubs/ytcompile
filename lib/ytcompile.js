 ...
 *
 * The templates file extension should be one of handlebars|hbs, micro|mu which
 * will determine which compilation step we take
 *
 * The <script> id attribute will be used to set the namespace on Y.U1.Template
 * for that template
 *
 * ./precompile -d /path/to/output file1 file2 file3
 *
 * -d output directory, optional, defaults to ./js/templates/
 *
 */
...
function parsedFileDomHandler( dom, type ) {
        ...
        for (i = 0, l = dom.length; i < l; i++ ) {
            ...
            if ( node.type === 'script' ) {
                if (
                    ( node.attribs.type === 'x-template' || node.attribs.type === 'text/x-handlebars-template' ) &&
                    node.children &&
